...
export default function PatientPortal() {
  ...
  // FIXED: Track authentication locally in memory. No localStorage leaks to cause hospital logout loops!
  ...
  // Dynamic identity profiling target state hooks
  ...
  useEffect(() => {
    ...
    if (pIdToken) {
      ...
      // Look up the database records dynamically based on the incoming URL parameter link context
      const matchedProfile = consents.find(c => c.id === parsedId);
      if (matchedProfile) {
        setActivePatientProfile({ id: matchedProfile.id, name: matchedProfile.name });
        setPhoneNumber(matchedProfile.phone);
      }
    }
    if (sourceHospital) {
  ...
  const patientRecords = consents.filter(c => c.id === activePatientProfile.id);
  const patientReports = reports.filter(r => r.name === activePatientProfile.name);

  const handleRequestOtp = (e) => {
    e.preventDefault();
    if (!phoneNumber) return alert("Please type your mobile line number.");
    setAuthStep(2);
  };

  const handleVerifyOtp = (e) => {
    e.preventDefault();
    if (!otpCode) return alert("Please enter your verification pin.");
    setIsLoggedIn(true);

    setActivities(prev => [{
      id: Date.now(),
      user: activePatientProfile.name,
      action: `Patient opened private vault files using link generated by ${senderName}`,
      time: 'Just now'
    }, ...prev]);
  };
  ...
  // --- LOGIN SECURITY SHELL (MOBILE FRIENDLY) ---
  if (!isLoggedIn) {
    return (
      <div className="min-h-screen w-full bg-slate-900 flex flex-col justify-between p-4 font-sans text-slate-100">
        <div className="w-full max-w-sm mx-auto bg-slate-850 p-6 rounded-2xl border border-slate-800 shadow-2xl space-y-6 my-auto">
          ...
          <div className="bg-teal-950/40 border border-teal-500/20 p-4 rounded-xl flex items-start space-x-3 text-left">
            ...
            <div className="space-y-0.5">
              <span className="text-[10px] font-bold tracking-wider uppercase text-teal-400 block">Secure Connection Profile</span>
              <p className="text-xs text-slate-200 leading-relaxed font-medium">
                Records link dispatched by: <strong className="text-white font-bold block text-sm mt-0.5">{senderName}</strong>
              </p>
            </div>
          </div>

          <div className="text-center space-y-1">
            <div className="w-10 h-10 bg-slate-800 border border-slate-700 rounded-full flex items-center justify-center mx-auto text-teal-400">
              <Shield className="w-5 h-5" />
            </div>
            <h2 className="text-base font-black text-white tracking-tight pt-1">Patient Identity Gate</h2>
            <p className="text-[11px] text-slate-400">Welcome back, <strong className="text-teal-400 font-bold">{activePatientProfile.name}</strong>. Please confirm your mobile line.</p>
          </div>

          {authStep === 1 ? (
            <form onSubmit={handleRequestOtp} className="space-y-4 text-left">
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-400 block pl-0.5">Mobile Phone Line</label>
            ...
            <form onSubmit={handleVerifyOtp} className="space-y-4 text-left">
              <div className="space-y-1.5">
                <div className="flex justify-between items-center px-0.5">
                  <label className="text-xs font-bold text-slate-400 block">4-Digit Verification OTP</label>
              ...
              <button type="submit" className="w-full py-3 bg-slate-100 text-slate-950 font-bold text-xs rounded-xl shadow-lg transition uppercase tracking-wide">
                Unlock My Records
              </button>
            </form>
          )}
        ...
        <div className="w-full text-center text-[10px] text-slate-600 font-bold tracking-wider uppercase py-2 shrink-0">
          ✓ DPDP Secure Vault Certification
        </div>
      </div>
    );
  }

  // --- LOGGED IN PORTAL INTERFACE VIEWPORT ---
  return (
    <div className="min-h-screen bg-slate-50 font-sans pb-12 text-slate-900 w-full overflow-x-hidden flex flex-col">
      ...
      <div className="max-w-md mx-auto w-full px-4 mt-4 space-y-5 flex-1">
        <div className="bg-teal-50/60 border border-teal-100 p-3.5 rounded-xl text-xs text-teal-950 leading-relaxed flex items-start space-x-2.5 shadow-sm text-left">
          ...
          <p>Welcome, <strong>{activePatientProfile.name}</strong>. Manage your security authorization preferences or review files sent securely by your provider below.</p>
        </div>
        ...
        <div className="space-y-2 text-left">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm divide-y divide-slate-100 overflow-hidden">
            {patientReports.length === 0 ? (
              <p className="p-4 text-slate-400 text-center text-xs">No medical report files have been issued to this profile link yet.</p>
            ) : (
