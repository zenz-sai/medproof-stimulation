...
export default function ReportDelivery() {
  const { consents, reports, setReports, setActivities } = useData();
  ...
  return (
    <div className="w-full space-y-6 animate-fadeIn text-slate-900">
      ...
      {showSendForm && (
        <form onSubmit={handleSendReport} className="bg-white p-5 rounded-xl border border-slate-200 shadow-md max-w-xl space-y-4 animate-slideDown">
          ...
          {/* Dynamic guardrail warning hint indicator */}
            ...
            <tbody className="divide-y divide-slate-100 text-xs">
              {reports.map((report) => (
                <tr key={report.id} className="hover:bg-slate-50/60 transition group">
                  ...
                  {/* Status timestamps chain */}
                  <td className="py-3.5 px-4 text-slate-500 text-[11px] space-x-2">
                  ...
                  {/* Verification Actions query logic */}
      ...
      {/* CRYPTOGRAPHIC CUSTODY TRAIL MODAL POPUP DETAILED CONTAINER */}
      {activeProof && (
        <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4 animate-fadeIn">
          <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-2xl max-w-md w-full font-sans text-slate-800">
            ...
            <div className="space-y-4 text-xs">
              ...
              <div className="space-y-2 border-l-2 border-teal-500 pl-3.5 ml-1 relative">
